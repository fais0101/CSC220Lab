	...
	*/
	...
	public void writeOutput() {
		...
		try {
			...
			for(int r = 0; r< height;r++) { //write the maze into a file
				output.println();
				for(int c =0; c < width; c++) {
					char toAdd = maze[r][c].content;
					output.print(toAdd);
				}
			}
			output.println();
	...
	private void buildGraph() {
		...
		try {
			...
			height = Integer.parseInt(fline[0]);
			width = Integer.parseInt(fline[1]);
			maze = new Node[height][width];
			
			for(int r = 0; r < height;r++) { //this will build our maze
